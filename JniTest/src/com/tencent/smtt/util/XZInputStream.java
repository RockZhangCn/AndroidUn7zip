...
/**
 * The {@code XZInputStream} class is used to read data stored in the XZ
 * format, reading and decompressing XZ data from the underlying stream into
 * its buffer.
 *
 * <h3>Example</h3>
 * <p>Using {@code XZInputStream} is easier than {@link ZipInputStream}
 * because XZ is only for compression, and is not a container for multiple files.
 * This code decompresses the data from a XZ stream, similar to the {@code gunzip(1)} utility.
 * <pre>
 * InputStream is = ...
 * XZInputStream zis = new XZInputStream(new BufferedInputStream(is));
 * try {
 ...
 * </pre>
 */
public class XZInputStream extends InflaterInputStream {
    ...
    /**
     * The magic header for the XZ format.
     */
    //public static final int XZ_MAGIC = 0x8b1f;
    ...
    /**
     * Construct a {@code XZInputStream} to read from XZ data from the
     * underlying stream.
     *
     * @param is
     ...
     */
    public XZInputStream(InputStream is) throws IOException {
        //Use this method.
        this(is, BUF_SIZE);
    }
     ...
     *             if an {@code IOException} occurs.
     */
    public XZInputStream(InputStream is, int size) throws IOException {
        super(is, new Inflater(), size);//InputStream is has the XZ header.
        /*
        byte[] header = new byte[10];
        readFully(header, 0, header.length);
        short magic = Memory.peekShort(header, 0, ByteOrder.LITTLE_ENDIAN);
        if (magic != (short) XZ_MAGIC) {
    ...
    /**
     * Reads and decompresses XZ data from the underlying stream into the
     * given buffer.
     */
    @Override
    public int read(byte[] buffer, int offset, int byteCount) throws IOException {
        ...
        XZUtil.checkOffsetAndCount(buffer.length, offset, byteCount);
